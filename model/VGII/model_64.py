...
class SCGBlock(nn.Module):
    ...
    def forward(self, y1, y2, d, sigma):
        ...
        for i in range(self.n_iter):
            ...
            # x1, x2 = self.SC_body(x1, x2, d, Y1, Y2, alpha_x, beta_x)   #[N,64,H,W]
...
def WeightGraph(x):
    ...
    weight_graph = (x - min_val) / (max_val - min_val + 1e-8)
    return weight_graph
...
class PIIG(nn.Module):
    def __init__(self, **kargs):
        super(PIIG, self).__init__()
    ...
    def forward(self, ir, vis, d, sigma, LLM_model, LLM_tokenizer):
        device1 = torch.device('cuda:0')
        d = d.repeat(ir.size(0), 1, 1, 1, 1)
        sigma = sigma[:1]
        ...
        weight_graph = WeightGraph(ir)
        ir_weight = ir_p * weight_graph

        return ir_p[:, :1, :, :], ir_sc, ir_p_sc, vis_sc, ir_weight